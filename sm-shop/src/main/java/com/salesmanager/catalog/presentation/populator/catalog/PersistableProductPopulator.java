...
/*				if(productAvailability.getId() != null && productAvailability.getId().longValue() == 0) {
			...
			if(source.getAttributes()!=null) {
				for(com.salesmanager.catalog.presentation.model.product.attribute.ProductAttributeEntity attr : source.getAttributes()) {
			...
			if(!CollectionUtils.isEmpty(source.getCategories())) {
				for(com.salesmanager.catalog.presentation.model.category.Category categ : source.getCategories()) {
